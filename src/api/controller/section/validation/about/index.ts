import { CreateSectionData, Education, Work } from "../../../../../shared-types/section";
...
export default class SectionValidationAbout {
	...
	static validateEducationData(data: CreateSectionData, formData: FormData) {
		...
		if (!data.educationData) throw new Error("you must at least inform an education");

		if (data.educationData.courses.length >= 1) {
			data.educationData.courses.forEach(SectionValidationAbout.validateCourseData);
		}
	...
	static validateWorkDataExperience(data: CreateSectionData, formData: FormData) {
        if (!data.workData) throw new Error("your about section must have some work data");

        if(data.workData.workExperience.length >= 1) {
            data.workData.workExperience.forEach(SectionValidationAbout.validateWorkData);
        }
	...
	static validateBiosData(data: CreateSectionData, formData: FormData) {
        ...
        if (!data.biosData.bios) throw new Error("your about section must have a resume bios about yourself");

        if(data.biosData.bios.length > 1000) throw new Error("your about section resume bios cannot have more than 1000 characters");
